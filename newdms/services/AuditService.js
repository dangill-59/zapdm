...
class AuditService {
    ...
    /**
     * Core logging method (alias for logAction) for compatibility
     */
    async log(...args) {
        return this.logAction(...args);
    }

    /**
     * Log user action (main implementation)
     */
    async logAction(action, details = {}) {
        ...
        try {
            ...
            // If using Sequelize or an ORM with create
            if (this.models.AuditLog && typeof this.models.AuditLog.create === 'function') {
            ...
            // If using a plain SQL insert (fallback)
            if (this.models.db) {
                const stmt = this.models.db.prepare(`
                    INSERT INTO audit_logs (action, entityType, entityId, userId, ipAddress, userAgent, details, createdAt)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                `);
                stmt.run(
                    auditData.action,
            ...
            throw new Error('No AuditLog model or db available for logging');
    ...
    async getAuditLogs(filters = {}) {
        ...
        try {
            ...
            if (action && this.models.Sequelize) whereConditions.action = { [this.models.Sequelize.Op.like]: `%${action}%` };
