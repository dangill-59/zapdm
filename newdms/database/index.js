// backend/newdms/database/index.js
// Complete rewrite for proper model instantiation with SQLite DB
// Updated to use environment/configurable DB path

const path = require('path');
const Database = require('better-sqlite3');
const Models = require('./models');

// --- Use DB path from config/environment or fallback ---
let dbPath;
try {
    // Try to load from config/environment if it exists
    // Adjust the path as needed for your project structure
    const envConfig = require('../../config/environment');
    dbPath = envConfig.DB_PATH || path.resolve(__dirname, '../../database/dms.sqlite');
} catch (e) {
    // Fallback if environment config is missing
    dbPath = path.resolve(__dirname, '../../database/dms.sqlite');
}
...
async function initialize() {
    // You can add other DB sync/init logic here if needed
